...
let message : string = (<string>nickname).length > 5 ? `Bienvenido ${nickname}` : `Nickname demasiado corto`;
console.log("nickname : ",nickname);

// ####### Sintaxis "as" #########

let otherMessage : string = (nickname as string).length > 5 ? `Bienvenido ${nickname}` : `Nickname demasiado corto`;
console.log("nickname : ",nickname);

let helloUser : any = "Hola Jordan";
let anotherMessage: string = (helloUser as string).substring(5);
...
let dni : any = 73436473;
let castDni : string = dni as string;  // Muestra un error en tiempo de ejecucion mas no de compilacion
let convertDni : string = String(dni); // Conversion correcta
